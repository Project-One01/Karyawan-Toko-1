...
function updateStockAfterSale(items) {
  items.forEach(function (cartItem) {
    ...
    allBatches.forEach((batch) => {
      ...
      batch.stokBM1 = (batch.stokKelompokBM1 || 0) * banyakItem + (batch.stokTurunanBM1 || 0);
      batch.stokBM2 = (batch.stokKelompokBM2 || 0) * banyakItem + (batch.stokTurunanBM2 || 0);
...
function performBatchPromotion(idBarang) {
  const mainBarangIndex = barangData.findIndex((b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru");
  ...
  // Update main barang dengan data dari batch lama
  barangData[mainBarangIndex] = {
    ...
    catatan: `✅ Batch Promoted | Dari: ${formatDate(oldestBatch.tanggal)} | Modal: ${formatRupiah(oldestBatch.modalBarang)}, Harga: ${formatRupiah(oldestBatch.hargaJualKelompok)}`,
  };

  // ✅ PERBAIKAN: Tandai batch untuk dihapus PERMANENT dari Google Sheets
  ...
  return {
    ...
    deleteId: oldestBatch.id, // ✅ ID untuk dihapus
  };
}
...
function getBarangToSave(affectedItemIds, promotionResults) {
  const toSave = [];
  const toDelete = []; // ✅ Array untuk info batch yang akan dihapus

  affectedItemIds.forEach((itemId) => {
    const mainBarang = barangData.find((b) => b.id === itemId && b.jenisTransaksi === "Penambahan Barang Baru");
    if (mainBarang) toSave.push(mainBarang);

    const promotionInfo = promotionResults.find((r) => r.itemId === itemId && r.promoted);

    if (promotionInfo) {
      // ✅ Cari batch yang sudah promoted untuk dihapus
      ...
      if (emptyBatch) {
        // ✅ Simpan info lengkap untuk delete
        toDelete.push({
          id: emptyBatch.id,
