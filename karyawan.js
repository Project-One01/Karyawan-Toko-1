...
function showLoading(show) {
  ...
  if (!loader && show) {
    ...
    loader.style.cssText =
      "position:fixed;top:0;left:0;right:0;height:3px;background:transparent;z-index:9999;overflow:hidden;";
...
async function loadAllData() {
  try {
    if (!isOnline()) {
      alert(
        "❌ Tidak ada koneksi internet. Aplikasi memerlukan koneksi untuk bekerja."
      );
      return false;
    }
    ...
    barangData = barang;
    transaksiData = Array.isArray(transaksi)
      ? transaksi
    ...
    alert(
      "❌ Gagal memuat data dari server. Silakan cek koneksi internet dan refresh halaman."
    );
    return false;
  } finally {
    showLoading(false);
    ...
    depositData.forEach((item) => {
      ...
      if (namaLower.includes(query)) {
        if (
          ...
          new Date(item.waktu) > new Date(customerMap[item.nama].waktu)
        ) {
...
function renderDepositSearchResults(results) {
  ...
  if (results.length === 0) {
    container.innerHTML =
      '<p class="search-empty">Pelanggan tidak ditemukan atau tidak memiliki deposit</p>';
    return;
  }
  ...
  container.innerHTML = results
    .map((item) => {
      ...
      return `<div class="search-item" onclick="selectDepositCustomer('${
        item.nama
      }', ${item.saldo})">
...
function renderSearchResults(results) {
  ...
  container.innerHTML = results
    .map(function (item) {
      ...
      const turunanLabel =
        item.jenisTurunan ||
        getSatuanTurunanLabel(item.jenisKelompok || "Satuan");

      // ✅ Ambil semua batch untuk barang ini (diurutkan dari lama ke baru = FIFO)
      const allBatches = barangData
        .filter((b) => b.id === item.id)
        .sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal));

      // Hitung total stok
      const totalStokSource = calculateTotalStokForBarang(
        item.id,
        barangData,
        STORE_ID
      );
      const totalStokBM1 = calculateTotalStokForBarang(
        item.id,
        barangData,
        "BM1"
      );
      const totalStokBM2 = calculateTotalStokForBarang(
        item.id,
        barangData,
        "BM2"
      );
      ...
      // ✅ Buat breakdown stok per batch untuk toko saat ini
      ...
      allBatches.forEach((batch) => {
        ...
        const batchTotalStok =
          batchStokKelompok * banyakItem + batchStokTurunan;

        // Skip batch yang stoknya 0
        if (batchTotalStok <= 0) return;

        // Tentukan label batch
        const batchLabel =
          batch.jenisTransaksi === "Penambahan Stok Lama"
            ? '<span style="color: #f59e0b; font-weight: 600;">Batch ' +
              batchNumber +
              "</span>"
      ...
      const stockWarning =
        totalStokSource < 20 ? ' <span class="stock-low">(Menipis)</span>' : "";

      // ✅ Template dengan breakdown batch
      return (
...
window.addToCart = function (itemId) {
  ...
  const hargaModalUnit =
    barang.modalBarang && banyakItem > 0
      ? barang.modalBarang / banyakItem
      : barang.modalBarang || 0;
  ...
  if (existingItem) {
    existingItem.qty++;
    // ✅ Update harga asli karena qty berubah
    ...
    cartItems.push({
      ...
      harga: defaultHarga,
      qty: 1,
      hargaAsli: defaultHarga, // ✅ SET HARGA ASLI saat first add
      diskonPersen: 0,
      diskonRupiah: 0,
      ...
      hargaModalUnit:
        defaultType === "kelompok" ? barang.modalBarang : hargaModalUnit,
      stokSource: STORE_ID,
...
function renderCart() {
  ...
  container.innerHTML = cartItems
    .map(function (item, index) {
      ...
      const maxStock = calculateTotalStokForBarang(
        item.id,
        barangData,
        item.stokSource
      );

      const hargaKelompok = barangAsli
        ? barangAsli.hargaJualKelompok
        : item.harga;
      const hargaTurunan = barangAsli
        ? barangAsli.hargaJualTurunan
        : item.harga;
      const isManual =
        item.harga !== hargaKelompok && item.harga !== hargaTurunan;

      const stokBadge =
        '<span class="stok-badge stok-' +
        item.stokSource.toLowerCase() +
        '">' +
        item.stokSource +
        "</span>";
      const kelompokLabel = getKelompokLabel(item.jenisKelompok || "Satuan");
      ...
      // ✅ FIFO Pricing Preview
      const fifoResult = calculateFIFOPricing(
        item.id,
        item.qty,
        item.stokSource
      );
      ...
      // ✅ PERBAIKAN: Hitung harga asli yang benar
      let hargaAsli = 0;

      // Jika item baru ditambahkan (belum pernah diedit), gunakan harga dari type
      if (!item.hargaAsli || item.hargaAsli === 0) {
        if (usesFIFO) {
          hargaAsli = fifoResult.totalCost;
        } else {
          // Tentukan harga asli berdasarkan type barang
        ...
        // Simpan harga asli ke item untuk referensi
        item.hargaAsli = hargaAsli;
      } else {
        // Jika sudah ada hargaAsli yang tersimpan, gunakan itu
        hargaAsli = item.hargaAsli;
      }

      // Harga aktual (setelah user ubah)
      const hargaAktual = item.harga * item.qty;

      // ✅ DISKON = Harga Asli - Harga Aktual (jika ada pengurangan)
      const diskonRupiah = Math.max(0, hargaAsli - hargaAktual);
      ...
      // ✅ TAMBAHKAN LOG DEBUG
      console.log("[RENDER_CART] Item:", item.id);
      console.log("  Harga Asli:", hargaAsli);
      console.log("  Harga Aktual:", hargaAktual);
      console.log("  Diskon Rp:", diskonRupiah);
      console.log("  Diskon %:", diskonPersen);

      const subtotal = hargaAktual;
      ...
      return (
        ...
        (diskonRupiah > 0
          ? '<div style="background: #dcfce7; border-left: 3px solid #16a34a; padding: 8px; margin-top: 8px; border-radius: 4px;">' +
            ...
            " → Harga Baru: " +
            formatRupiah(hargaAktual) +
            "</div>" +
            "</div>"
          : "") +
        // ✅ Subtotal dengan tampilan diskon
...
window.updateSubtotal = function (index, value) {
  ...
  // ✅ Hitung harga per unit berdasarkan subtotal yang diinput
  ...
  if (barangAsli) {
    ...
    item.hargaModalUnit =
      barangAsli.modalBarang && banyakItem > 0
        ? barangAsli.modalBarang / banyakItem
        : barangAsli.modalBarang || 0;
  }

  // ✅ AUTO CALCULATE DISKON
  // Ambil harga asli dari data attribute atau dari perhitungan
  const hargaAsli =
    parseFloat(
      document.querySelector('.subtotal-input[data-index="' + index + '"]')
        .dataset.hargaAsli
    ) ||
    item.hargaAsli ||
    newSubtotal;
  ...
  // Reset manual grand total
  manualGrandTotal = null;

  console.log("[UPDATE_SUBTOTAL] Item: " + item.id);
  console.log("  Harga Asli: " + formatRupiah(hargaAsli));
  console.log("  Subtotal Baru: " + formatRupiah(newSubtotal));
  console.log(
    "  Diskon: " +
      formatRupiah(diskonRupiah) +
      " (" +
  ...
  console.log("  ✅ Diskon tersimpan di item:", {
...
window.changeItemPrice = function (index, priceValue) {
  ...
  const banyakItem = barangAsli.banyakItemPerTurunan || 1;
  const hargaModalUnit = isKelompok
    ? barangAsli.modalBarang
    : barangAsli.modalBarang && banyakItem > 0
    ? barangAsli.modalBarang / banyakItem
    : barangAsli.modalBarang || 0;
  ...
  // ✅ PERBAIKAN: Set harga asli baru saat ganti type
  if (!isManual) {
...
window.updateManualPrice = function (index, value) {
  ...
  if (!isNaN(newPrice) && newPrice >= 0) {
    ...
    // ✅ PERBAIKAN: Jika hargaAsli belum di-set, set sekarang
    if (!item.hargaAsli || item.hargaAsli === 0) {
      if (barangAsli) {
        const hargaOriginal =
          item.typeBarang === "kelompok"
            ? barangAsli.hargaJualKelompok
            : barangAsli.hargaJualTurunan;
    ...
    const diskonPersen =
      item.hargaAsli > 0 ? (diskonRupiah / item.hargaAsli) * 100 : 0;

    item.diskonRupiah = diskonRupiah;
    item.diskonPersen = diskonPersen;

    console.log("[UPDATE_PRICE] Item: " + item.id);
    console.log("  Harga Asli Total: " + formatRupiah(item.hargaAsli));
    console.log("  Harga Baru Total: " + formatRupiah(hargaBaru));
    console.log(
      "  Diskon: " +
        formatRupiah(diskonRupiah) +
        " (" +
...
window.increaseQty = function (index) {
  ...
  if (cartItems[index].qty < maxStock) {
    ...
    // ✅ Update harga asli
    const hargaPerUnit =
      cartItems[index].hargaAsli / (cartItems[index].qty - 1);
...
window.updateQty = function (index, value) {
  ...
  if (qty > 0 && qty <= maxStock) {
    ...
    // ✅ Reset manual grand total karena qty berubah
    manualGrandTotal = null;
...
function updateCartTotal() {
  // ✅ PERBAIKAN: Hitung total dari subtotal aktual (bukan harga × qty)
  const calculatedTotal = cartItems.reduce(function (sum, item) {
    // Hitung FIFO jika ada multiple batches
    ...
    // Gunakan FIFO total jika multiple batches, kalau tidak pakai harga × qty
    const itemSubtotal = usesFIFO
  ...
  const finalTotal =
    manualGrandTotal !== null ? manualGrandTotal : calculatedTotal;
  ...
  // ✅ PERBAIKAN: Tampilkan "Kembalian" atau "Kurang" dengan benar
  const changeEl = document.getElementById("changeAmount");
  if (changeEl) {
    if (difference >= 0) {
      changeEl.textContent = formatRupiah(difference);
      changeEl.style.color = "#10b981"; // Hijau untuk kembalian
    } else {
      changeEl.textContent = formatRupiah(Math.abs(difference));
      changeEl.style.color = "#ef4444"; // Merah untuk kurang
    }
  }
...
window.updateGrandTotal = function (value) {
  ...
  // ✅ Hitung total asli (tanpa diskon manual grand total)
  let totalAsli = 0;
  cartItems.forEach(function (item) {
    ...
    const itemHargaAsli =
      item.hargaAsli ||
      (usesFIFO ? fifoResult.totalCost : item.harga * item.qty);
  ...
  if (diskonGrandTotal > 0) {
    console.log("[GRAND_TOTAL] Total Asli: " + formatRupiah(totalAsli));
    console.log("[GRAND_TOTAL] Total Baru: " + formatRupiah(newTotal));
    console.log("[GRAND_TOTAL] Diskon: " + formatRupiah(diskonGrandTotal));

    // Distribusikan diskon ke semua item secara proporsional
    cartItems.forEach(function (item) {
      const fifoResult = calculateFIFOPricing(
        item.id,
        item.qty,
      ...
      const itemDiskonPersen =
        itemHargaAsli > 0 ? (itemDiskon / itemHargaAsli) * 100 : 0;
...
function getUpdatedBarangFromCart(cartItems) {
  const updated = [];
  ...
  cartItems.forEach(function (item) {
    ...
    // ✅ PERBAIKAN: Ambil HANYA barang utama
    const mainBarang = barangData.find(function (b) {
      return b.id === item.id && b.jenisTransaksi === "Penambahan Barang Baru";
    });

    if (mainBarang) {
      updated.push(mainBarang);
    }
...
/**
 * Cek apakah batch utama habis, jika ya promote batch lama jadi utama
 * @param {string} idBarang - ID barang yang akan dicek
 */
async function promoteBatchIfNeeded(idBarang) {
  console.log("[PROMOTE] Checking batch for:", idBarang);

  // 1️⃣ Cari barang utama
  const mainBarang = barangData.find(
    (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru"
  );

  if (!mainBarang) {
    console.log("[PROMOTE] Main barang not found:", idBarang);
    return false;
  }

  // 2️⃣ Hitung total stok utama (BM1 + BM2)
  const banyakItem = mainBarang.banyakItemPerTurunan || 1;

  const stokMainBM1 =
    (mainBarang.stokKelompokBM1 || 0) * banyakItem +
    (mainBarang.stokTurunanBM1 || 0);
  const stokMainBM2 =
    (mainBarang.stokKelompokBM2 || 0) * banyakItem +
    (mainBarang.stokTurunanBM2 || 0);
  const totalStokMain = stokMainBM1 + stokMainBM2;

  console.log("[PROMOTE] Main stock check:", {
    BM1: stokMainBM1,
    BM2: stokMainBM2,
    total: totalStokMain,
  });

  // ✅ Jika masih ada stok, tidak perlu promosi
  if (totalStokMain > 0) {
    console.log("[PROMOTE] Main batch still has stock, no promotion needed");
    return false;
  }

  console.log("[PROMOTE] ⚠️ Main batch EMPTY! Looking for oldest batch...");

  // 3️⃣ Cari batch lama tertua yang masih ada stoknya
  const oldBatches = barangData
    .filter(
      (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Stok Lama"
    )
    .sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal)); // FIFO: lama → baru

  console.log("[PROMOTE] Found", oldBatches.length, "old batches");

  // Cari batch pertama yang masih punya stok
  let oldestBatch = null;

  for (const batch of oldBatches) {
    const batchStokBM1 =
      (batch.stokKelompokBM1 || 0) * banyakItem + (batch.stokTurunanBM1 || 0);
    const batchStokBM2 =
      (batch.stokKelompokBM2 || 0) * banyakItem + (batch.stokTurunanBM2 || 0);
    const batchTotal = batchStokBM1 + batchStokBM2;

    console.log(
      "[PROMOTE] Checking batch from",
      batch.tanggal,
      "- Stock:",
      batchTotal
    );
    ...
    if (batchTotal > 0) {
      oldestBatch = batch;
      break; // Ambil yang pertama (tertua)
    }
  }

  if (!oldestBatch) {
    console.log("[PROMOTE] ❌ No batch available to promote (all empty)");
    return false;
  }

  console.log("[PROMOTE] ✅ Promoting batch from:", oldestBatch.tanggal);
  console.log("[PROMOTE] Batch data:", {
    modal: oldestBatch.modalBarang,
    hargaKelompok: oldestBatch.hargaJualKelompok,
    hargaTurunan: oldestBatch.hargaJualTurunan,
    stokKelompokBM1: oldestBatch.stokKelompokBM1,
    stokKelompokBM2: oldestBatch.stokKelompokBM2,
    stokTurunanBM1: oldestBatch.stokTurunanBM1,
    stokTurunanBM2: oldestBatch.stokTurunanBM2,
  });

  // 4️⃣ UPDATE MAIN: Timpa dengan data batch lama
  const updatedMain = {
    tanggal: mainBarang.tanggal, // ✅ Tetap pakai tanggal awal
    id: mainBarang.id,
    nama: mainBarang.nama,
    jenisKelompok: mainBarang.jenisKelompok,
    banyakItemPerTurunan: mainBarang.banyakItemPerTurunan,
    jenisTurunan: mainBarang.jenisTurunan,

    // ✅ PROMOSI: Gunakan stok dari batch lama
    ...
    // ✅ PROMOSI: Gunakan harga dari batch lama
    ...
    // Statistik tetap dari main (jangan direset)
    totalItemTerjual: mainBarang.totalItemTerjual || 0,
    totalHargaItemTerjual: mainBarang.totalHargaItemTerjual || 0,
    totalModal: mainBarang.totalModal || 0,
    keuntunganMargin: mainBarang.keuntunganMargin || 0,

    jenisTransaksi: "Penambahan Barang Baru",
    alamat: mainBarang.alamat || "-",
    status: "Tambah Barang Baru",
    catatan: `✅ Batch Promoted | Dari: ${formatDate(
      oldestBatch.tanggal
    )} | Modal: ${formatRupiah(oldestBatch.modalBarang)}, Harga: ${formatRupiah(
      oldestBatch.hargaJualKelompok
    )}`,
  };

  // 5️⃣ UPDATE BATCH LAMA: Kosongkan stok (sudah dipindahkan)
  const updatedOldBatch = {
    ...
    catatan:
      (oldestBatch.catatan || "") +
      " | ✅ PROMOTED to main batch on " +
      formatDate(getTodayWIB()),
  };

  try {
    // 6️⃣ SAVE ke server
    console.log("[PROMOTE] Saving to server...");
    const response = await saveBarangData([updatedMain, updatedOldBatch]);

    if (!response || !response.success) {
      throw new Error("Server save failed");
    }

    console.log("[PROMOTE] ✅ Server save successful");

    // 7️⃣ UPDATE local data (barangData array)
    const mainIndex = barangData.findIndex(
      (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru"
    );

    if (mainIndex !== -1) {
      barangData[mainIndex] = updatedMain;
      console.log("[PROMOTE] ✅ Updated main in local data");
    }

    const batchIndex = barangData.findIndex(
      (b) =>
        b.id === oldestBatch.id &&
        b.tanggal === oldestBatch.tanggal &&
        b.jenisTransaksi === "Penambahan Stok Lama"
    );

    if (batchIndex !== -1) {
      barangData[batchIndex] = updatedOldBatch;
      console.log("[PROMOTE] ✅ Updated old batch in local data");
    }

    console.log("[PROMOTE] 🎉 Batch promotion complete!");
    console.log("[PROMOTE] New main stock:", {
      BM1:
        (updatedMain.stokKelompokBM1 || 0) * banyakItem +
        (updatedMain.stokTurunanBM1 || 0),
      BM2:
        (updatedMain.stokKelompokBM2 || 0) * banyakItem +
        (updatedMain.stokTurunanBM2 || 0),
      modal: updatedMain.modalBarang,
      harga: updatedMain.hargaJualKelompok,
    });

    return true;
  } catch (error) {
    console.error("[PROMOTE ERROR]", error);
    alert("⚠️ Gagal mempromosikan batch: " + error.message);
    return false;
  }
}
...
function getBarangToSave(affectedItemIds, promotionResults) {
  ...
  console.log("[GET_BARANG_TO_SAVE] Processing", affectedItemIds.size, "items");

  affectedItemIds.forEach((itemId) => {
    // 1. Selalu kirim main barang
    const mainBarang = barangData.find(
      (b) => b.id === itemId && b.jenisTransaksi === "Penambahan Barang Baru"
    );
    if (mainBarang) {
      toSave.push(mainBarang);
      console.log("[GET_BARANG_TO_SAVE] Added MAIN:", itemId);
    }

    // 2. Cek apakah item ini di-promote
    const promotionInfo = promotionResults.find(
      (r) => r.itemId === itemId && r.promoted
    );

    if (promotionInfo) {
      // ✅ PERBAIKAN: Cari batch menggunakan promotion ID yang unik
      ...
      if (emptyBatch) {
        toSave.push(emptyBatch);
        console.log(
          "[GET_BARANG_TO_SAVE] Added PROMOTED BATCH:",
          itemId,
          "(ID:",
        ...
        console.warn(
          "[GET_BARANG_TO_SAVE] ⚠️ Empty batch not found for",
          itemId
        );
      }
    } else {
      // Jika TIDAK di-promote, kirim semua batch yang ter-update
  ...
  console.log("[GET_BARANG_TO_SAVE] Total records to save:", toSave.length);
  return toSave;
}

// ============================================
// FULL IMPLEMENTATION: initCheckout dengan FIFO Promotion
// Konsisten antara frontend dan backend, TIDAK buat row baru
// ============================================
...
async function initCheckout() {
  ...
  btnCheckout.addEventListener("click", async () => {
    ...
    if (!isOnline()) {
      alert(
        "❌ Tidak ada koneksi internet. Checkout memerlukan koneksi online."
      );
      return;
    }

    // ============================================
    // 2. HITUNG TOTAL dengan FIFO
    // ============================================
    ...
    if (useDepositForTransaction && selectedDepositCustomer) {
      ...
      if (finalTotal > customerBalance) {
        ...
        if (payment < shortfall) {
          alert(
            "❌ Pembayaran tunai kurang! Harus bayar: " +
              formatRupiah(shortfall)
          );
          return;
        }
      ...
      // ✅ FITUR BARU: Izinkan pembayaran kurang dengan status "Belum Lunas"
      const kekurangan = finalTotal - payment;

      // ✅ PERBAIKAN: Validasi jika payment = 0, wajib isi
      if (payment === 0) {
        alert(
    ...
    // ✅ DEBUG LOG - Tambahkan setelah validasi
    console.log("[CHECKOUT] Payment Info:", {
      finalTotal,
      payment,
      sisa: Math.max(0, finalTotal - payment),
      status: payment >= finalTotal ? "Lunas" : "Belum Lunas",
    });

    // ============================================
    // 4. BUAT RINGKASAN BARANG untuk Transaksi
    // ============================================
    ...
    cartItems.forEach(function (item) {
      ...
      if (usesFIFO) {
        fifoResult.breakdown.forEach(function (batch, idx) {
          const batchLabel =
            batch.batch === "Stok Lama" ? " [Batch Lama]" : " [Batch Awal]";
          barangListForKeuangan +=
            item.nama +
            " (" +
            batch.qty +
            " " +
            ...
            batchLabel +
            ") [" +
            (item.stokSource || STORE_ID) +
            "], ";
          totalBanyaknya += batch.qty;
        });
      } else {
        const unitLabel =
          item.typeBarang === "kelompok" ? kelompokLabel : turunanLabel;
        barangListForKeuangan +=
          item.nama +
          " (" +
          item.qty +
    ...
    if (useDepositForTransaction && selectedDepositCustomer) {
      ...
      transactionNote =
        "Kurang Bayar: " + formatRupiah(sisaTagihan) + ". " + transactionNote;
    }

    // ✅ Tambahkan info diskon di catatan jika ada
    if (totalDiskon > 0) {
      transactionNote =
        "Total Diskon: " + formatRupiah(totalDiskon) + ". " + transactionNote;
    }

    if (useDepositForTransaction && selectedDepositCustomer) {
      transactionNote =
        "Bayar dengan Deposit: " +
        formatRupiah(Math.min(finalTotal, selectedDepositCustomer.saldo)) +
        ". " +
        transactionNote;
    }
    ...
    const transaction = {
      ...
      total: finalTotal,
      bayar: payment, // ✅ Kolom "Bayar"
      sisa: sisaTagihan, // ✅ Kolom "Sisa"
    ...
    // ✅ DEBUG LOG - Cek data sebelum dikirim
    console.log("[CHECKOUT] Transaction before save:", {
      id: transaction.id,
    ...
    // ============================================
    // 6. BUAT DETAIL TRANSAKSI dengan FIFO Breakdown
    // ============================================
    ...
    cartItems.forEach(function (item) {
      ...
      if (usesFIFO) {
        fifoResult.breakdown.forEach(function (batch) {
          ...
          const batchDiskonRupiah =
            totalSebelumDiskon > 0
              ? (batchSubtotal / totalSebelumDiskon) * diskonRupiah
              : 0;
    ...
    // ============================================
    // 7. KUMPULKAN SEMUA BARANG yang Akan Diupdate
    // ============================================
    const affectedItemIds = new Set();
    cartItems.forEach((item) => affectedItemIds.add(item.id));

    console.log("[CHECKOUT] Affected items:", affectedItemIds.size);

    // ============================================
    // 8. UPDATE STOK di MEMORY (barangData)
    // ============================================
    console.log("[CHECKOUT] Updating stock in memory...");
    updateStockAfterSale(cartItems);

    // ============================================
    // 9. CEK & PROMOSI BATCH (SEBELUM SAVE)
    // ============================================
    console.log("[CHECKOUT] Checking batch promotions...");
    ...
    for (const itemId of affectedItemIds) {
      ...
      if (needsPromotion) {
        console.log("[CHECKOUT] Item", itemId, "needs batch promotion");
    ...
    console.log("[CHECKOUT] Promotions:", promotionResults);

    // ============================================
    // 10. KUMPULKAN ULANG BARANG SETELAH PROMOSI
    // ============================================
    const finalBarangToSave = getBarangToSave(
      affectedItemIds,
      promotionResults
    );
    console.log("[CHECKOUT] Final records to save:", finalBarangToSave.length);
    ...
    try {
      ...
      console.log("[CHECKOUT] Step 1: Saving transaction...");
      await saveTransaksiData(transaction, detailTransactionData);

      console.log("[CHECKOUT] Step 2: Saving barang data...");
      const saveResult = await saveBarangData(finalBarangToSave);
      console.log("[CHECKOUT] Save result:", saveResult);

      // ============================================
      // 12. DEPOSIT LOGIC
      // ============================================
      if (useDepositForTransaction && selectedDepositCustomer) {
        console.log("[CHECKOUT] Step 3: Processing deposit...");
        ...
        const depositTransaction = {
          ...
          saldo: newBalance,
          keterangan:
            "Belanja di " + STORE_ID + " - Total: " + formatRupiah(finalTotal),
      ...
      // ============================================
      // 13. REFRESH DATA dari SERVER
      // ============================================
      console.log("[CHECKOUT] Step 4: Refreshing data from server...");
      const [barang, transaksi, deposit] = await Promise.all([
        loadBarangData(),
        loadTransaksiData(),
        loadDepositData(),
      ...
      console.log(
        "[CHECKOUT] ✅ Data refreshed! Total barang:",
        barangData.length
      );

      // ============================================
      // 14. HITUNG KEMBALIAN & SHOW SUCCESS
      // ============================================
      const actualPayment =
        useDepositForTransaction && selectedDepositCustomer
      ...
      if (useDepositForTransaction && selectedDepositCustomer) {
        ...
        alertMessage +=
          "Sisa Deposit: " +
          formatRupiah(selectedDepositCustomer.saldo - depositUsed) +
          "\n";
      } else {
        ...
        if (sisaTagihan > 0) {
          ...
          alertMessage +=
            "\n💡 Klik status 'Belum Lunas' di tabel untuk melunaskan.";
        }
      }
      ...
      alert(
        "❌ Gagal menyimpan transaksi. Silakan cek koneksi dan coba lagi.\n\nError: " +
          error.message
      );
    } finally {
      showLoading(false);
...
// ============================================
// HELPER: Cek apakah perlu promosi
// ============================================
...
/**
 * Lakukan batch promotion di memory (TIDAK save ke server)
 * @param {string} idBarang - ID barang yang akan dipromosikan
 * @returns {boolean} True jika berhasil dipromosikan
 */
function performBatchPromotion(idBarang) {
  console.log("[PERFORM_PROMOTION] Starting for:", idBarang);

  const mainBarangIndex = barangData.findIndex(
    (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru"
  );

  if (mainBarangIndex === -1) {
    console.log("[PERFORM_PROMOTION] Main not found");
    return false;
  }
  ...
  // ✅ PERBAIKAN: Cari batch dengan stok DAN simpan index-nya
  let oldBatchIndex = -1;

  for (let i = 0; i < barangData.length; i++) {
    const b = barangData[i];

    if (b.id !== idBarang || b.jenisTransaksi !== "Penambahan Stok Lama") {
      continue;
    }

    const batchStokBM1 =
      (b.stokKelompokBM1 || 0) * banyakItem + (b.stokTurunanBM1 || 0);
    const batchStokBM2 =
      (b.stokKelompokBM2 || 0) * banyakItem + (b.stokTurunanBM2 || 0);
    const batchTotal = batchStokBM1 + batchStokBM2;
  ...
  if (oldBatchIndex === -1) {
    console.log("[PERFORM_PROMOTION] No batch with stock available");
    return false;
  }

  const oldestBatch = barangData[oldBatchIndex];

  // ✅ TAMBAHKAN: Unique identifier untuk tracking
  const promotionTimestamp = new Date().getTime();

  console.log("[PERFORM_PROMOTION] Promoting batch at index:", oldBatchIndex);
  console.log("[PERFORM_PROMOTION] Batch date:", oldestBatch.tanggal);

  // 3. TIMPA main dengan data batch
  ...
  // 4. KOSONGKAN batch lama + TAMBAHKAN identifier unik
  barangData[oldBatchIndex] = {
    ...
    totalItem: 0,
    status: "Batch Selesai",
    // ✅ PERBAIKAN: Tambahkan timestamp unik
    catatan:
      (oldestBatch.catatan || "") +
      ` | ✅ PROMOTED_${promotionTimestamp} to main on ` +
      formatDate(getTodayWIB()),
    // ✅ TAMBAHAN: Simpan tanggal asli untuk tracking
    promotedDate: oldestBatch.tanggal,
  };

  console.log("[PERFORM_PROMOTION] ✅ Promotion completed");
  console.log("[PERFORM_PROMOTION] Promotion ID:", promotionTimestamp);
...
async function saveBatchPromotionToServer(idBarang) {
  console.log("[SAVE_PROMOTION] Saving batch promotion for:", idBarang);

  // Cari main dan batch yang baru di-promote
  const mainBarang = barangData.find(
    (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru"
  );

  // Cari batch yang baru dikosongkan (status = Batch Selesai, totalItem = 0)
  const emptyBatch = barangData.find(
    (b) =>
      b.id === idBarang &&
      b.jenisTransaksi === "Penambahan Stok Lama" &&
      ...
      b.totalItem === 0 &&
      b.catatan &&
      b.catatan.includes("PROMOTED")
  );

  if (!mainBarang || !emptyBatch) {
    console.error("[SAVE_PROMOTION] Missing main or empty batch!");
    return false;
  }

  console.log("[SAVE_PROMOTION] Sending 2 records to server");
  console.log("[SAVE_PROMOTION] Main:", {
    id: mainBarang.id,
    tanggal: mainBarang.tanggal,
    modal: mainBarang.modalBarang,
    harga: mainBarang.hargaJualKelompok,
    totalItem: mainBarang.totalItem,
  });
  console.log("[SAVE_PROMOTION] Empty Batch:", {
    id: emptyBatch.id,
    tanggal: emptyBatch.tanggal,
    totalItem: emptyBatch.totalItem,
    status: emptyBatch.status,
  });

  try {
    // ✅ KIRIM HANYA 2 RECORDS: main + empty batch
    const result = await saveBarangData([mainBarang, emptyBatch]);

    if (!result || !result.success) {
      throw new Error("Server save failed");
    }

    console.log("[SAVE_PROMOTION] ✅ Server save successful");
    return true;
  } catch (error) {
    console.error("[SAVE_PROMOTION ERROR]", error);
    return false;
  }
}
...
function getBarangToSave(affectedItemIds, promotionResults) {
  ...
  affectedItemIds.forEach((itemId) => {
    // Selalu kirim main barang
    const mainBarang = barangData.find(
      (b) => b.id === itemId && b.jenisTransaksi === "Penambahan Barang Baru"
    );
    if (mainBarang) toSave.push(mainBarang);

    // Cek apakah item ini di-promote
    const wasPromoted = promotionResults.find(
      (r) => r.itemId === itemId && r.promoted
    );

    if (wasPromoted) {
      // Kirim batch yang dikosongkan
      const emptyBatch = barangData.find(
        (b) =>
          b.id === itemId &&
          b.jenisTransaksi === "Penambahan Stok Lama" &&
      ...
      if (emptyBatch) toSave.push(emptyBatch);
    } else {
      // Kirim semua batch yang ter-update (stok berkurang tapi tidak jadi 0)
      const updatedBatches = barangData.filter(
        (b) =>
          b.id === itemId &&
          b.jenisTransaksi === "Penambahan Stok Lama" &&
          b.totalItem >= 0 // Semua batch (kosong atau tidak)
      );
      toSave.push(...updatedBatches);
    }
  });
...
/**
 * Verifikasi konsistensi data setelah operasi
 * @param {Set} affectedItemIds - Set ID barang yang terpengaruh
 */
function verifyDataConsistency(affectedItemIds) {
  console.log(
    "[VERIFY] Checking consistency for",
  ...
  affectedItemIds.forEach((itemId) => {
    const allRecords = barangData.filter((b) => b.id === itemId);
    const mainRecord = allRecords.find(
      (b) => b.jenisTransaksi === "Penambahan Barang Baru"
    );
    const batchRecords = allRecords.filter(
      (b) => b.jenisTransaksi === "Penambahan Stok Lama"
    );

    if (!mainRecord) {
      issues.push(`❌ Item ${itemId}: Main record not found`);
      return;
    }

    // Cek negative stock
    if (mainRecord.totalItem < 0) {
      issues.push(
        `⚠️ Item ${itemId}: Negative stock (${mainRecord.totalItem})`
      );
    }

    // Cek batch consistency
    const emptyBatchesWithStock = batchRecords.filter(
      (b) => b.totalItem > 0 && b.status === "Batch Selesai"
    );

    if (emptyBatchesWithStock.length > 0) {
      issues.push(
        `⚠️ Item ${itemId}: ${emptyBatchesWithStock.length} batches marked as done but have stock`
      );
    }

    console.log("[VERIFY]", itemId, {
      mainStock: mainRecord.totalItem,
      batches: batchRecords.length,
      modal: mainRecord.modalBarang,
      harga: mainRecord.hargaJualKelompok,
    });
  });

  if (issues.length > 0) {
    console.warn("[VERIFY] Found issues:", issues);
    return { success: false, issues };
  }

  console.log("[VERIFY] ✅ All data consistent");
 ...
 */
function logTransactionDetails(transaction, items) {
  console.group("💰 TRANSACTION DETAILS");
  console.log("ID:", transaction.id);
  console.log("Customer:", transaction.nama);
  console.log("Total:", formatRupiah(transaction.total));
  console.log("Items:", items.length);

  items.forEach((item, idx) => {
    console.log(
      `  ${idx + 1}. ${item.nama} - ${item.qty} ${
        item.typeBarang
 ...
 */
function logStockChanges(itemId, before, after) {
  console.group(`📦 STOCK CHANGE: ${itemId}`);
  console.log("Before:", before);
  console.log("After:", after);
  console.log("Diff:", {
    BM1: after.BM1 - before.BM1,
    BM2: after.BM2 - before.BM2,
    total: after.BM1 + after.BM2 - (before.BM1 + before.BM2),
  });
  console.groupEnd();
}

/**
 * Log batch promotion untuk debugging
 */
function logBatchPromotion(itemId, mainBarang, oldBatch) {
  console.group(`🔄 BATCH PROMOTION: ${itemId}`);
  console.log("From Batch:", formatDate(oldBatch.tanggal));
  console.log("Modal:", formatRupiah(oldBatch.modalBarang), "→ Main");
  console.log("Harga:", formatRupiah(oldBatch.hargaJualKelompok), "→ Main");
  console.log("Stock Moved:", oldBatch.totalItem);
  console.groupEnd();
}
...
/**
 * Handle error dengan recovery
 */
async function handleCheckoutError(error, context) {
  ...
  const errorMessage =
    `❌ Terjadi kesalahan saat ${context}.\n\n` +
    `Detail: ${error.message}\n\n` +
    `Silakan cek koneksi internet dan coba lagi.\n` +
  ...
  try {
    console.log("[ERROR_RECOVERY] Reloading data...");
    await loadAllData();
    console.log("[ERROR_RECOVERY] ✅ Data reloaded");
  } catch (reloadError) {
    console.error("[ERROR_RECOVERY] Failed to reload:", reloadError);
  }
}
...
function updateStockAfterSale(items) {
  items.forEach((cartItem) => {
    ...
    const barangUtama = barangData.find(
      (b) => b.id === idBarang && b.jenisTransaksi === "Penambahan Barang Baru"
    );
    ...
    if (
      cartItem.typeBarang === "kelompok" &&
      jenisKelompok.toLowerCase() !== "satuan"
    ) {
      qtyNeeded = cartItem.qty * banyakItem;
    }

    // ✅ FIFO: Ambil semua batch (utama + history)
    const allBatches = barangData
      .filter((b) => b.id === idBarang)
      .sort((a, b) => new Date(a.tanggal) - new Date(b.tanggal));
    ...
    allBatches.forEach((batch) => {
      ...
      // ✅ UPDATE stok di memory (modifikasi object barangData)
      if (stokSource === "BM1") {
    ...
    // ✅ PERBAIKAN: Update Total Item di barang utama
    const totalStokBaru = calculateTotalStokForBarang(
      idBarang,
      barangData,
      stokSource
    );
...
function renderTodayTransactionsTable() {
  ...
  if (filtered.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" class="table-empty">Belum ada transaksi hari ini di ' +
      STORE_ID +
      ".</td></tr>";
    return;
  }
  ...
  tbody.innerHTML = filtered
    .map((t) => {
      if (t.tipeProses === "refund" || t.tipeProses === "exchange") {
        ...
        const fullBarangList =
          refundItemsText +
          (refundItemsText && exchangeItemsText ? "<br>" : "") +
          exchangeItemsText;
      ...
      if (t.jenis === "Pemasukan") {
        ...
        // ✅ KLIK STATUS "BELUM LUNAS"
        const statusClickable =
          t.status === "Belum Lunas"
            ? 'onclick="lunaskanTransaksi(' +
              t.id +
              ')" style="cursor: pointer;" title="Klik untuk melunaskan"'
            : "";
        ...
        return (
          ...
          infoBayar + // ✅ Tampilkan info bayar/sisa
          "</td>" +
          '<td><span class="' +
          statusClass +
          '" ' +
...
window.lunaskanTransaksi = async function (transactionId) {
  if (!isOnline()) {
    alert(
      "❌ Tidak ada koneksi internet. Pelunasan memerlukan koneksi online."
    );
    return;
  }

  // ✅ PERBAIKAN: Convert ke string untuk comparison yang konsisten
  const searchId = transactionId.toString();

  // Cari transaksi dengan loose comparison
  const transaction = transaksiData.find((t) => t.id.toString() === searchId);

  if (!transaction) {
    alert("❌ Transaksi tidak ditemukan!");
    console.error("[LUNASKAN] Transaction not found. ID:", searchId);
    console.log(
      "[LUNASKAN] Available IDs:",
      transaksiData.map((t) => t.id)
    );
    return;
  }
  ...
  if (pembayaran < sisaTagihan) {
    alert("❌ Pembayaran kurang! Sisa tagihan: " + formatRupiah(sisaTagihan));
    return;
  }

  const kembalian = pembayaran - sisaTagihan;
  ...
  try {
    ...
    const updatedTransaction = {
      ...transaction,
      status: "Lunas",
      bayar: totalBelanja, // ✅ Update total bayar
      sisa: 0, // ✅ Sisa jadi 0
      catatan:
        (transaction.catatan || "") +
        " | ✅ Dilunaskan: " +
        formatRupiah(sisaTagihan) +
        " pada " +
        formatDateTime(getNowWIB()) +
        (kembalian > 0 ? " (Kembalian: " + formatRupiah(kembalian) + ")" : ""),
    };

    // Save ke server (hanya update header transaksi)
    ...
    console.error("[LUNASKAN ERROR]", error);
    alert(
      "❌ Gagal melunaskan transaksi. Silakan coba lagi.\n\nError: " +
        error.message
    );
  } finally {
    showLoading(false);
...
window.printInvoice = async function (transactionIdOrName) {
  ...
  let transaction = transaksiData.find(
    (t) => t.id.toString() === searchId && t.stokSource === STORE_ID
  );

  if (
    !transaction &&
    typeof transactionIdOrName === "string" &&
    isNaN(transactionIdOrName)
  ) {
    ...
    const matches = transaksiData.filter(
      (t) =>
        ...
        t.nama.toLowerCase().includes(searchName)
    );
  ...
  if (transaction.stokSource !== STORE_ID) {
    alert(
      "❌ Transaksi ini dilakukan di " +
        transaction.stokSource +
        ", tidak bisa dicetak di " +
        STORE_ID +
        "!"
    );
    return;
  ...
  // ✅ FUNGSI HELPER untuk format angka tanpa desimal
  ...
  const alamatToko =
    STORE_ID === "BM1"
      ? "Dsn V (P7), Desa Sei Alim Ulu, Kec. Air Batu"
      : "Jalinsum Desa Air Teluk Hessa, Kec. Teluk Dalam";

  const noFaktur = transaction.noStruk || transaction.id;
  const tglFaktur = formatDate(transaction.tanggal);
  const namaPelanggan =
    transaction.nama && transaction.nama.trim() !== ""
      ? transaction.nama
      : "NO NAME";
  const alamatPelanggan =
    transaction.alamat &&
    transaction.alamat.trim() !== "" &&
    transaction.alamat !== "-"
      ? transaction.alamat
      : "-";
  ...
  const jumlahDibayar = transaction.bayar || 0; // ✅ Jangan gunakan fallback grandTotal
  const sisaTagihan = transaction.sisa || 0;
  ...
  printWindow.document.write(
    "<!DOCTYPE html>" +
      ...
      ";" +
      "background: " +
      (statusPembayaran === "Lunas" ? "#d1fae5" : "#fef3c7") +
      ";" +
      "padding: 10px;" +
      "margin: 15px 0;" +
      "border-radius: 5px;" +
...
function renderListBarangTable(data) {
  ...
  if (data.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="9" class="table-empty">Tidak ada data barang yang ditemukan.</td></tr>';
    return;
  }
  ...
  tbody.innerHTML = data
    .map((item) => {
      ...
      const stokKelompokToko =
        STORE_ID === "BM1"
          ? item.stokKelompokBM1 || 0
          : item.stokKelompokBM2 || 0;
      const stokTurunanToko =
        STORE_ID === "BM1"
          ? item.stokTurunanBM1 || 0
          : item.stokTurunanBM2 || 0;
      const totalStokToko = stokKelompokToko * banyakItem + stokTurunanToko;

      const totalStokBM1 =
        (item.stokKelompokBM1 || 0) * banyakItem + (item.stokTurunanBM1 || 0);
      const totalStokBM2 =
        (item.stokKelompokBM2 || 0) * banyakItem + (item.stokTurunanBM2 || 0);
...
function resetRefundProcess() {
  ...
  const refundTransactionTableBody = document.getElementById(
    "refundTransactionTableBody"
  );
  const searchResultsRefund = document.getElementById("searchResultsRefund");

  if (refundTransactionTableBody) {
    refundTransactionTableBody.innerHTML =
      '<tr><td colspan="5" class="table-empty">Cari transaksi untuk memulai proses.</td></tr>';
  }
  if (searchResultsRefund) searchResultsRefund.innerHTML = "";

  // Reset display dan totals TANPA trigger infinite loop
...
function updateRefundDisplay(type) {
  ...
  const diffRowSpan = document
    .getElementById("refundDiffRow")
    ?.querySelector("span");
  ...
  if (diffRowSpan) {
    diffRowSpan.textContent = isRefund
      ? "Uang Kembali (Nilai Dikembalikan):"
      : "Selisih (Uang Keluar/Masuk):";
  }
}
...
function renderRefundTransactionList(results) {
  ...
  if (results.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="5" class="table-empty">Transaksi tidak ditemukan.</td></tr>';
    return;
  }
...
window.selectTransactionForRefund = async function (transactionIdOrName) {
  ...
  const refundOriginalTransInfo = document.getElementById(
    "refundOriginalTransInfo"
  );
...
function renderRefundItems() {
  ...
  if (allItems.length === 0) {
    container.innerHTML =
      '<p class="refund-empty">Tidak ada barang dalam proses.</p>';
    updateRefundTotals(); // ✅ HANYA panggil saat kosong
    return;
  }
  ...
  container.innerHTML = allItems
    .map((item, globalIndex) => {
      ...
      const localIndex = isRefund
        ? globalIndex
        : globalIndex - refundItems.length;
      ...
      const maxStock = isRefund
        ...
        : (() => {
            ...
            const stokKelompok =
              item.stokSource === "BM1"
                ? barang.stokKelompokBM1 || 0
                : barang.stokKelompokBM2 || 0;
            const stokTurunan =
              item.stokSource === "BM1"
                ? barang.stokTurunanBM1 || 0
                : barang.stokTurunanBM2 || 0;
            return stokKelompok * banyakItem + stokTurunan;
          })();

      const borderClass = isRefund
        ? "refund-item-border"
        : "exchange-item-border";
      const labelQty = isRefund ? "Dikembalikan" : "Dibeli";
      const labelHeader = isRefund
        ? "(Awal - Dikembalikan)"
        : "(Tambah - Dibeli)";
      const updateQtyFunc = isRefund
        ? "updateRefundItemQty(" + localIndex + ", this.value)"
        : "updateExchangeItemQty(" + localIndex + ", this.value)";
      const updatePriceFunc = isRefund
        ? "updateRefundItemPrice(" + localIndex + ", this.value)"
        : "updateExchangeItemPrice(" + localIndex + ", this.value)";
      const removeFunc = isRefund
        ? "removeRefundItem(" + localIndex + ")"
        : "removeExchangeItem(" + localIndex + ")";
  ...
  // ❌ JANGAN panggil updateRefundTotals() di sini!
}
...
window.updateRefundItemQty = function (index, value) {
  ...
  if (qty > 0 && qty <= originalQty) {
    ...
    renderRefundItems();
    updateRefundTotals(); // ✅ Urutan: render dulu, baru update totals
  } else if (qty > originalQty) {
    alert(
      "❌ Jumlah pengembalian tidak boleh melebihi pembelian awal (" +
        originalQty +
        ")!"
    );
...
window.updateRefundItemPrice = function (index, value) {
  ...
  if (!isNaN(newPrice) && newPrice >= 0) {
    ...
    renderRefundItems();
    updateRefundTotals(); // ✅ Update totals setelah render
  } else {
    alert("❌ Harga tidak valid!");
    renderRefundItems();
  }
...
window.removeRefundItem = function (index) {
  ...
  renderRefundItems(); // ✅ Render dulu
  updateRefundTotals(); // ✅ Baru update totals
};

window.updateExchangeItemQty = function (index, value) {
  const qty = parseInt(value);
  if (qty > 0) {
    const barangAsli = barangData.find(
      (b) => b.id === addedExchangeItems[index].id
    );
    if (!barangAsli) {
    ...
    const stokKelompok =
      stokSource === "BM1"
        ? barangAsli.stokKelompokBM1 || 0
        : barangAsli.stokKelompokBM2 || 0;
    const stokTurunan =
      stokSource === "BM1"
        ? barangAsli.stokTurunanBM1 || 0
        : barangAsli.stokTurunanBM2 || 0;
...
function renderRefundAddSearchResults(results) {
  ...
  if (results.length === 0) {
    container.innerHTML =
      '<p class="search-empty-small">Barang tidak ditemukan</p>';
    return;
  }
...
function updateRefundTotals() {
  ...
  // Hitung total nilai barang yang dikembalikan
  const refundTotalValue = refundItems.reduce(
    (sum, item) => sum + item.harga * item.qty,
    0
  );

  // Hitung total nilai barang pengganti (hanya untuk exchange)
  const exchangeTotalValue = isRefund
    ? 0
    : addedExchangeItems.reduce((sum, item) => sum + item.harga * item.qty, 0);

  // Hitung selisih (positif = pelanggan bayar tambahan, negatif = pelanggan terima uang)
  const netDifference = exchangeTotalValue - refundTotalValue;

  // Update tampilan total refund
  const refundRefundTotal = document.getElementById("refundRefundTotal");
  if (refundRefundTotal) {
    refundRefundTotal.textContent = formatRupiah(refundTotalValue);
  }

  // Update tampilan total exchange
  const refundExchangeTotal = document.getElementById("refundExchangeTotal");
  if (refundExchangeTotal) {
    refundExchangeTotal.textContent = formatRupiah(exchangeTotalValue);
  }

  // Update tampilan selisih dengan styling yang sesuai
  const refundDifference = document.getElementById("refundDifference");
  if (refundDifference) {
    if (isRefund) {
      // Mode refund: tampilkan uang yang dikembalikan
      refundDifference.textContent = formatRupiah(refundTotalValue);
      refundDifference.className = "total-danger";
    } else {
      // Mode exchange: tampilkan selisih
      refundDifference.textContent = formatRupiah(Math.abs(netDifference));

      if (netDifference > 0) {
        // Pelanggan menambah uang
        refundDifference.className = "total-success";
      } else if (netDifference < 0) {
        // Pelanggan menerima uang kembali
        refundDifference.className = "total-danger";
      } else {
        // Tukar setara
        refundDifference.className = "total-neutral";
  ...
  // ❌ JANGAN panggil renderRefundItems() di sini!
}
...
async function finishRefundProcess() {
  ...
  if (!isOnline()) {
    alert(
      "❌ Tidak ada koneksi internet. Proses refund/tukar memerlukan koneksi online."
    );
    return;
  }
  ...
  const note =
    refundNote?.value ||
    "Proses " +
      (isRefund ? "Pengembalian" : "Penukaran") +
      " Barang dari " +
      STORE_ID +
      ".";
  ...
  if (isRefund && refundTotalValue > activeRefundTransaction.total) {
    alert(
      "❌ Nilai barang yang dikembalikan tidak boleh melebihi total transaksi awal."
    );
    return;
  }
  ...
  // ============================================
  // 5. KUMPULKAN AFFECTED ITEMS (SEBELUM UPDATE)
  // ============================================
  const affectedItemIds = new Set();
  refundItems.forEach((item) => affectedItemIds.add(item.id));
  addedExchangeItems.forEach((item) => affectedItemIds.add(item.id));

  console.log("[REFUND] Affected items:", affectedItemIds.size);

  // ============================================
  // 6. BUAT RINGKASAN BARANG
  // ============================================
  ...
  const refundTrans = {
    ...
    barang:
      barangListRefund +
      (addedExchangeItems.length > 0 ? ", " + barangListExchange : ""),
  ...
  // Kurangi qty barang yang di-refund
  ...
  // Tambahkan barang exchange (jika ada)
  ...
  // ============================================
  // 9. BUAT DETAIL TRANSAKSI
  // ============================================
  const detailRefundData = refundItems
    .concat(addedExchangeItems)
    .map(function (item) {
      return {
  ...
  // ============================================
  // 10. UPDATE STOK di MEMORY
  // ============================================
  console.log("[REFUND] Updating stock in memory...");

  // Update stok untuk barang yang di-refund (tambah kembali)
  updateStockAfterRefund(refundItems);

  // Update stok untuk barang exchange (kurangi stok)
  if (!isRefund) {
    updateStockAfterSale(addedExchangeItems);
  }

  // ============================================
  // 11. CEK & PROMOSI BATCH (SEBELUM SAVE)
  // ============================================
  console.log("[REFUND] Checking batch promotions...");
  const promotionResults = [];
  ...
  for (const itemId of affectedItemIds) {
    ...
    if (needsPromotion) {
      console.log("[REFUND] Item", itemId, "needs batch promotion");
      const promotionInfo = performBatchPromotion(itemId);
  ...
  console.log("[REFUND] Promotions:", promotionResults);

  // ============================================
  // 12. KUMPULKAN ULANG BARANG SETELAH PROMOSI
  // ============================================
  const finalBarangToSave = getBarangToSave(affectedItemIds, promotionResults);
  console.log("[REFUND] Final records to save:", finalBarangToSave.length);
  ...
  try {
    ...
    console.log("[REFUND] Step 1: Saving NEW refund transaction...");
    await saveTransaksiData([refundTrans], detailRefundData);

    console.log("[REFUND] Step 2: Updating ORIGINAL transaction...");
    // ✅ UPDATE transaksi utama
    await saveTransaksiData([originalTrans], []); // Tanpa detail, hanya update header

    // ✅ UPDATE detail transaksi original di Google Sheets
    console.log("[REFUND] Step 3: Updating ORIGINAL transaction details...");
    await updateDetailTransaksi(originalTrans.id, newItemsOriginal);

    console.log("[REFUND] Step 4: Saving barang data...");
    const saveResult = await saveBarangData(finalBarangToSave);
    console.log("[REFUND] Save result:", saveResult);

    // ============================================
    // 14. REFRESH DATA dari SERVER
    // ============================================
    console.log("[REFUND] Step 5: Refreshing data from server...");
    const [barang, transaksi, deposit] = await Promise.all([
      loadBarangData(),
      loadTransaksiData(),
      loadDepositData(),
    ...
    console.log("[REFUND] ✅ Data refreshed! Total barang:", barangData.length);

    // ============================================
    // 15. VERIFIKASI KONSISTENSI
    // ============================================
    console.log("[REFUND] Verifying data consistency...");
    for (const itemId of affectedItemIds) {
      const allRecords = barangData.filter((b) => b.id === itemId);
      const mainRecord = allRecords.find(
        (b) => b.jenisTransaksi === "Penambahan Barang Baru"
      ...
      if (mainRecord) {
        console.log("[VERIFY]", itemId, "- Main stock:", mainRecord.totalItem);
      }
    }

    // ============================================
    // 16. RENDER ULANG TABEL TRANSAKSI ✅
    // ============================================
    console.log("[REFUND] Step 4: Rendering transaction table...");
    renderTodayTransactionsTable();
    ...
    const transaksiNav = document.querySelector(
    ...
    alert(
      "❌ Gagal memproses refund/tukar. Silakan cek koneksi dan coba lagi.\n\nError: " +
        error.message
    );
  } finally {
    showLoading(false);
...
function updateStockAfterRefund(items) {
  items.forEach(function (rItem) {
    ...
    if (stokSource === "BM1") {
      const totalStokBM1 =
        (barang.stokKelompokBM1 || 0) * banyakItem +
        (barang.stokTurunanBM1 || 0);
      ...
      const totalStokBM2 =
        (barang.stokKelompokBM2 || 0) * banyakItem +
        (barang.stokTurunanBM2 || 0);
    ...
    barang.stokBM1 =
      (barang.stokKelompokBM1 || 0) * banyakItem + (barang.stokTurunanBM1 || 0);
    barang.stokBM2 =
      (barang.stokKelompokBM2 || 0) * banyakItem + (barang.stokTurunanBM2 || 0);
  });
}

window.printInvoiceByName = function () {
  const customerName = prompt(
    "Masukkan nama pelanggan untuk mencetak invoice:"
  );
...
window.selectRefundByName = function () {
  const customerName = prompt(
    "Masukkan nama pelanggan untuk proses refund/tukar:"
  );
  if (!customerName || customerName.trim() === "") {
    alert("❌ Nama pelanggan tidak boleh kosong!");
    return;
